...
const PatientLIst = () => {
  ...
  useEffect(() => {
    fetch(
      "http://localhost/ip2_project/aastu_clinic/backend/API/Doctor_dashboard/patients.php",
      {
        method: "GET",
        credentials: "include",
      }
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {patients.map((patient) => (
                <tr key={patient.id}>
                  <td className="px-4 py-2">
                    <div className="w-12 h-12 overflow-hidden border border-gray-200 rounded-full mx-auto">
                      <img
                        src={
                          patient.profilePic
                            ? `http://localhost/ip2_project/aastu_clinic/backend/public/${patient.profilePic}`
                            : "/default-profile.png"
                        }
                        alt="profile"
                  ...
                  <td className="px-4 py-2 text-gray-800">{patient.gender}</td>
                  <td className="px-4 py-2 text-gray-800">{patient.age}</td>
